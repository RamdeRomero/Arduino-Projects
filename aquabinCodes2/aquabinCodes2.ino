...
void setup() {
  ...
  // Gsm setup Gsm setup  Gsm setup Gsm setup
      ...
      rtc.setTime(3, 59, 1);   // Set the time to 12:00:00 (24hr format)
      rtc.setDate(5, 11, 2023); // Set the date to August 6th, 2019 
      //for millis timer
  ...
  if(digitalRead(Float_Switch) == HIGH){
    ...
    //##########################
      ...
      if(cm<=170 && (cm>160)){
        ...
        mySerial.println("AT+CMGS=\"+639639874383\"");//change ZZ with country code and xxxxxxxxxxx with phone number to sms
        updateSerial();
        mySerial.print("The bin is full...reply BIN OFF-to stop | visit http://192.168.45.147/ to monitor the aquabin"); //text content
        updateSerial();
        mySerial.write(26);
...
//      }
      ...
      Serial.println("Float 2 is off ");
      //send message
        mySerial.println("AT+CMGF=1"); // Configuring TEXT mode
        updateSerial();
        mySerial.println("AT+CMGS=\"+639639874383\"");//change ZZ with country code and xxxxxxxxxxx with phone number to sms
        updateSerial();
        mySerial.print("The bin water level 2 is Insufficient Check bin "); //text content
        updateSerial();
        mySerial.write(26);
        delay(30000);
